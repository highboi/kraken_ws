...
async function Tsunami(attributes=null) {
	...
	tsunamiDB.signalSocket = new WebSocket(`ws://${window.location.hostname}:8000/signal`);
	...
	tsunamiDB.makeNewConnection = async (peerid) => {
		...
		console.log(newConnection.connection.iceConnectionState);
	...
	tsunamiDB.addCandidates = async (connectionObj) => {
		console.log("ADDING ICE CANDIDATES TO CONNECTIONS");
